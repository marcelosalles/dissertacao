...
\begin{equation}
\label{eq:Cpdiff}
RMSE_{Cp} = \sqrt{\frac{\sum_{i=1}^{4}{\sum_{j=0}^{11}}{\sum_{k=0}^{N_d}{(Cp^{TPU}_{f_i,\alpha_j,p_k} - Cp^{MA}_{f_i,\alpha_j}})^2}}{48}}
\end{equation}
...
Não há uma recomendação específica para determinar o tamanho da amostra, mas os resultados são mais adequados para amostras maiores. Portanto, o número de casos foi escolhido de acordo com o tempo disponível e a capacidade de processamento do computador onde foram desenvolvidas as simulações.
Os parâmetros variados e seus limites mínimos e máximos foram definidos pela metodologia do item \ref{subsec:par}. 
...
O metamodelo foi desenvolvido por meio de \acrfull{ann}, utilizando-se a biblioteca \textit{TensorFlow} \cite{tensorflow2015}, disponibilizada para \citeonline{Python}.

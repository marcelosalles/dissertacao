...
%		
		...
		Enquanto que pelo método analítico (MA) os Cp's podem ser obtidos para quaisquer razões entre as dimensões das fachadas da edificação, os valores medidos em túnel de vento pela TPU são fornecidos para edificações com geometrias específicas.  % Ambas as fontes têm aplicação limitada a edificações retangulares.
...
%		O modelo preliminar sofreu variação dos seguintes parâmetros: área, razão entre largura e profundidade da zona, pé-direito, azimute, absortância, PAF, taxa de ocupação. Os demais parâmtros foram fixados de acordo com a Tabela \ref{table:paramfix}.
		...
		para cada \text{timestep}. Ainda assim, o cálculo apresentado é uma aproximação do que é calculado pelo algorítimo do programa.

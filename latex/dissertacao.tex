...
Diante deste cenário, uma ferramento capaz de estimar o conforto térmico em edificações de forma simples e rápida pode ser de grande utilidade.

O objetivo deste estudo é desenvolver um metamodelo de rede neural artificial capaz de estimar o conforto térmico em escritórios de edificações ventiladas naturalmente. 
...
O treinamento da rede neural artificial é realizado com uma uma base de dados de 100.000 simulações termoenergéticas, amostradas pelo métode de amostragem do hipercubo latino. 
...
\textabstract {
	The world's energy demand for air cooling in buildings is significant, and it is expected to increase in the next decades.
	A solution to mitigate the energy use for air cooling in the use of natural ventilation (NV).
	NV is as passive cooling strategy with significant potential for hot climates.
	Despite its potential, the use of NV has being decreasing in recent years for office buildings in Brazil, since the design of buildings with air conditioning prevails.
	
	It is important to conceive the use of NV since the early-design phases of the building to guarantee its effectiveness.
	...
	The aim of this study is to develop a neural networks surrogate model to estimate thermal comfort in naturally ventilated office building.
	The annual fraction of occupied hours with operative temperatures above the upper limit of ASHRAE's Standards 55 (2017) adaptive model, for 80\% of acceptability, is used as at thermal comfort index.
	
	The surrogate model is developed from a data set of building performance simulations, using the software EnergyPlus.
	Simulation models were defined based on a database of naturally ventilated office buildings in the city of Sao Paulo.
	The variables used as input in the surrogate model are defined by Sobol's sensitivity analysis.
	
	From data of 100,000 simulations, sampled by the Latin hypercube sampling method, the neural network was trained.
	...
	The final surrogate model achieved estimates for thermal comfort in naturally ventilated office buildings in the city of Sao Paulo with results close to the simulations developed on the software EnergyPlus. This surrogate model can be used by building designers as a simple tool to support decision making in early-design phases.
}
...
De acordo com \citeonline{Costola2010}, os \acrshort{cp} médios se definem de acordo com Equação \ref{eq:CalcCp} e Equação \ref{eq:Pvel}.
...
A \acrshort{as} foi aplicada por meio de programação, utilizando-se a biblioteca \textit{SALib} \cite{Herman2017}, escrita na linguagem \citeonline{Python}.
...
Partindo-se de uma tipologia com proporções de largura, profundidade e altura igual a 2:1:2, as diferenças nos resultados de simulações termoenergéticas com \acrshort{cp} obtidos pelos diferentes métodos foram avaliadas (Figura \ref{fig:cpaverage_ACH_scatter}). %, apresentada na ,

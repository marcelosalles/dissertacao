...
def process_folder(folder):
    ...
    # epjson_filesa = sorted(glob.glob('020*.epJSON'))  
    # epjson_filesb = sorted(glob.glob('050*.epJSON'))
    # epjson_files = epjson_filesa+epjson_filesb
    df_temp = {
        'folder': [],
        'file': [],
        'zone': [],
    ...
    # df_output.to_csv('means_{}_2.csv'.format(folder_name), index=False)
    df_output.to_csv('means_{}_3.csv'.format(folder_name), index=False)

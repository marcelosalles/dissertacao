...
NAME_STDRD = 'whole'
NAME_STDRD_2 = 'outdoors'
NAME_STDRD_3 = 'adiabatic'
INPUT_WHOLE = "single_zone/seed_noafn.json"
INPUT_SZ = "single_zone/seed_sznoafn.json"
EXTENSION = 'epJSON'
...
for i in range(len(sample)):
    ...
    for i in range(6):
        ...
        output = (FOLDER+'/cluster'+'{:01.0f}'.format(cluster_n)+'/'+NAME_STDRD_2+'_{}_'.format(case)+str(i)+'.epJSON')
        df = df.append(pd.DataFrame([sample_line+['cluster'+'{:01.0f}'.format(cluster_n),NAME_STDRD_2+'_{}_'.format(case)+str(i)+'.epJSON'.format(case)]],columns=col_names+['folder','file']))
        ...
        output = (FOLDER+'/cluster'+'{:01.0f}'.format(cluster_n)+'/'+NAME_STDRD_3+'_{}_'.format(case)+str(i)+'.epJSON')
        df = df.append(pd.DataFrame([sample_line+['cluster'+'{:01.0f}'.format(cluster_n),NAME_STDRD_3+'_{}_'.format(case)+str(i)+'.epJSON'.format(case)]],columns=col_names+['folder','file']))
        singlezone_diss.main(
            zone_area = model_values['area'], 
            zone_ratio = model_values['ratio'],
            zone_height = model_values['zone_height'],
...
print('\nPROCESSING OUTPUT\n')
output_processing2.main(df, MONTH_MEANS, OUTPUT_PROCESSED,ach=False)
